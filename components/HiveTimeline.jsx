      ...
      {steps.map((step, i) => (
        <li key={step.title} className="relative py-5">
          <Reveal delay={i * 60}>
            ...
            >
              <div
                className={`z-10 flex h-14 w-14 shrink-0 items-center justify-center rounded-full text-2xl shadow-soft sm:absolute sm:left-1/2 sm:-translate-x-1/2 ${
                  dark ? "bg-honey-400" : "bg-white ring-1 ring-honey-300"
                }`}
              >
                {step.emoji}
              </div>
              <div className={`sm:w-[calc(50%-3.5rem)] ${i % 2 === 1 ? "sm:mr-auto" : "sm:ml-auto"}`}>
                <p className={`text-xs font-bold tracking-wider2 ${dark ? "text-honey-300" : "text-honey-700"}`}>
